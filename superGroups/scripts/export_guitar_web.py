...
for bass,keys,drums in itertools.product(range(3),repeat=3):
    ...
    np.savez_compressed(a.out/f'rehearsal-{key}.npz',roll=roll)
    roll=trim_warmup(roll,64)
    write_midi(roll,path,110,['bass','keys','trancefusion guitarist','drums']);mid=mido.MidiFile(path)
    for msg in mid.tracks[3]:
        if msg.type=='program_change':msg.program=27
    mid.save(path)
    extra[key]={'tracks':[read_track(mid,r+1) for r in range(4)],'warmup_bars':4,'rehearsal_rounds':1,'steps':192,'bpm':110,'lineup':[bass,3+keys,12,9+drums],'guitar':True,'backing':'Fixed looped rhythm-section recording; learned guitar response'}
